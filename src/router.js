  ...
  return amendmentResult => {
    ...
    if (element) {
      return runCallbackIfPossible(element[amendmentFunction],
        Object.assign({cancel: () => ({cancel: true})}, context, {next: undefined}), element);
    }
  };
}
...
export class Router extends Resolver {
  ...
  __runOnBeforeCallbacks(newContext) {
    ...
    for (let i = newContext.__divergedChainIndex; i < newChain.length; i++) {
      const newItem = newChain[i];
      const prevItem = previousChain[i];
      // check if the parameters changed for the same route
      if (prevItem && prevItem.route === newItem.route && prevItem.path !== newItem.path) {
        // if element created by `context.component()` from action, use that, else create new one
        if (prevItem.element && !newItem.element) {
          newChain[i].element = createElement(newContext, prevItem.element.tagName);
        }
      }
